...
def load_manifest() -> dict:
    try:
        ...
        raise SystemExit("FEHLER: Runtime-Manifest ist nicht lesbar.") from exc
    if data.get("schema_version") != 1 or not isinstance(data.get("files"), list):
        raise SystemExit("FEHLER: Runtime-Manifest hat ein unbekanntes Schema.")
    files = data["files"]
    if len(files) != len(set(files)) or not all(isinstance(item, str) and item for item in files):
        raise SystemExit("FEHLER: Runtime-Manifest enthält ungültige oder doppelte Dateipfade.")
    return data


def main() -> None:
    parser = argparse.ArgumentParser(description="Sichere Runtime-Vorprüfung")
    parser.add_argument("--quick", action="store_true", help="kompakte Startprüfung")
    parser.parse_args()

    manifest = load_manifest()
    for rel in manifest["files"]:
        check((ROOT_DIR / rel).is_file(), f"Basisdatei {rel} vorhanden")

    check(bool(VERSION), "Version vorhanden")
    registry = validate_registry(json.loads((ROOT_DIR / "VERSION_REGISTRY.json").read_text(encoding="utf-8")))
    check(registry["current_version"] == VERSION, "VERSION und Registry stimmen überein")
    check(WEB_DIR.is_dir(), "Weboberfläche vorhanden")
