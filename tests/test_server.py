...
class SecurityContractTests(unittest.TestCase):
    def test_loopback_host_allowed_only_with_exact_port(self):
        self.assertTrue(allowed_host("127.0.0.1:8765",8765))
        self.assertTrue(allowed_host("localhost:8765",8765))
        self.assertFalse(allowed_host("127.0.0.1",8765))
        self.assertFalse(allowed_host("localhost",8765))
    ...
    def test_foreign_origin_and_cross_port_blocked(self):
        self.assertFalse(allowed_origin("https://example.com",8765))
        self.assertFalse(allowed_origin("http://127.0.0.1:9999",8765))
